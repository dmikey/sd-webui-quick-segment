...
# Global lock for body detection to prevent concurrent execution
_body_detect_lock = threading.Lock()
...
BODY_PRESETS = {
    ...
    "Legs": {"prompt": "leg . legs . feet", "threshold": 0.25},
...
#        print("⏳ Another detection task is running, waiting...")
    ...
    try:
        ...
        if body_preset == "Custom":
            ...
            preset_data = BODY_PRESETS.get(body_preset, BODY_PRESETS["Full Body"])
            text_prompt = preset_data["prompt"]
            threshold = box_threshold if box_threshold != 0.3 else preset_data["threshold"]
        ...
        masks, _, _ = predictor.predict_torch(
            ...
            point_labels=None,
            boxes=transformed_boxes.to(sam_device),
            multimask_output=False)  # Single best mask per detection
        
        masks = masks.cpu().numpy()
        boxes_filt_np = boxes_filt.numpy().astype(int) if boxes_filt is not None else None
        ...
        # We process largest masks first and try to merge smaller ones into them
        all_indices = list(range(num_detections))
        all_indices.sort(key=lambda idx: np.sum(masks[idx, 0, :, :]), reverse=True)
        ...
        for i in all_indices:
            ...
            current_box = boxes_filt_np[i] if boxes_filt_np is not None else None
            current_area = np.sum(current_mask)
            merged_indices.add(i)
            
            # Check for overlaps with remaining masks
            for j in all_indices:
                if j in merged_indices:
                    continue
                
                other_mask = masks[j, 0, :, :]
                other_area = np.sum(other_mask)
                ...
                if intersection > 0:
                    ...
                    # Metric 2: Containment (Intersection over Area of Smaller Mask)
                    # This helps merge small parts (heads/hands) that are mostly inside/touching the body
                    min_area_masks = min(current_area, other_area)
                    ...
                    if iou > 0.25 or containment > 0.75:
                        ...
                        # Update box to encompass both
                        if current_box is not None and boxes_filt_np is not None:
                            other_box = boxes_filt_np[j]
            ...
            # Step 2: Filter by area AFTER merging
            # This allows small parts (like heads) to be kept if they were merged into a body
        ...
        for i, individual_mask in enumerate(final_masks):
            ...
            if dilation_amt > 0:
                _, individual_mask = dilate_mask(individual_mask, dilation_amt)
            
            # Create visualization with box for this detection only
            mask_viz = show_masks(image_np.copy(), individual_mask[None, ...])
            if final_boxes and i < len(final_boxes):
                mask_viz = show_boxes(mask_viz, np.array([final_boxes[i]]))
            
            # Create mask image (white on black)
            mask_image = Image.fromarray((individual_mask * 255).astype(np.uint8))
            
            # Create matted image (transparent background)  
            matted = image_np.copy()
            matted[~individual_mask] = np.array([0, 0, 0, 0])
            
            # Add to gallery: [preview, mask, cutout] for each detection
            gallery_output.extend([
                Image.fromarray(mask_viz),
                mask_image,
                Image.fromarray(matted)
            ])
        
        # Update num_detections to reflect filtered count
        num_detections = len(final_masks)
        
        # If combine_masks is checked, also add combined mask at the end
        if combine_masks and num_detections > 1:
            # Combine only valid masks
            combined_mask = np.any(np.array(final_masks), axis=0)
            if dilation_amt > 0:
                _, combined_mask = dilate_mask(combined_mask, dilation_amt)
            
            combined_viz = show_masks(image_np.copy(), combined_mask[None, ...])
            if final_boxes:
                combined_viz = show_boxes(combined_viz, np.array(final_boxes))
            combined_mask_img = Image.fromarray((combined_mask * 255).astype(np.uint8))
            combined_matted = image_np.copy()
            combined_matted[~combined_mask] = np.array([0, 0, 0, 0])
            
            gallery_output.extend([
                Image.fromarray(combined_viz),
                combined_mask_img,
                Image.fromarray(combined_matted)
            ])
        ...
        status = f"✅ Detected {num_detections} body region(s) using '{body_preset}' preset."
        if not install_success:
            status += " Note: Using local GroundingDINO fallback."
        
        return gallery_output, gr.update(choices=mask_choices, value=mask_choices[0], visible=True), mask_choices, status
...
#        print(f"❌ Error in quick_body_detect: {e}")
        ...
        _body_detect_lock.release()


def get_selected_body_mask(gallery, selected_mask, mask_choices):
    """Get the mask image based on user selection."""
    if not gallery or not selected_mask or not mask_choices:
        return None
    
    try:
        # Find the index of the selected mask
        if isinstance(mask_choices, list):
            idx = mask_choices.index(selected_mask)
        else:
            idx = 0
        
        # Each detection has 3 images: [preview, mask, cutout]
        # The mask is at position idx * 3 + 1
        mask_idx = idx * 3 + 1
        
        if mask_idx < len(gallery):
            return Image.open(gallery[mask_idx]['name']).convert('L')
    except (ValueError, IndexError, KeyError) as e:
        pass  # Silently handle errors in mask selection
    
    return None


def copy_body_mask_to_inpaint(input_image, gallery, selected_mask, mask_choices, invert_mask):
    """Copy the selected body mask to img2img inpaint upload tab."""
#    print(f"[copy_body_mask_to_inpaint] Called with: input_image={type(input_image)}, gallery_len={len(gallery) if gallery else 0}, selected={selected_mask}")
    
    if input_image is None:
        return None, None, "❌ No input image available. Upload an image first."
    
    if not gallery or len(gallery) == 0:
        return None, None, "❌ No masks generated yet. Click 'Generate Body Mask' first."
    ...
    try:
        ...
        if selector in choices:
            body_idx = choices.index(selector)
        else:
            body_idx = 0
        
        # Each detection has 3 images: [preview, mask, cutout]
        # The mask is at position idx * 3 + 1
        mask_idx = body_idx * 3 + 1
        
        if mask_idx < len(gallery):
            mask = Image.open(gallery[mask_idx]['name']).convert('L')
            
            # Invert if requested (for inpainting the background)
            if invert_mask:
                from PIL import ImageOps
                mask = ImageOps.invert(mask)
            
            # Make a copy and convert to RGB for img2img
            if input_image is None:
            ...
            if img_copy.mode == 'RGBA':
                ...
                rgb_image.paste(img_copy, mask=img_copy.split()[3] if len(img_copy.split()) > 3 else None)
                img_copy = rgb_image
            elif img_copy.mode != 'RGB':
                img_copy = img_copy.convert('RGB')
            
            return img_copy, mask, f"✅ '{selector}' ready! Enable the checkbox below, then go to img2img → Inpaint Upload → Generate."
        else:
            return None, None, f"❌ Error: Mask index {mask_idx} out of range (gallery has {len(gallery)} items)"
...
def apply_body_preset(preset_name, current_threshold):
    """Update the prompt and threshold based on selected preset."""
    if preset_name == "Custom":
        return gr.update(visible=True), current_threshold
    preset_data = BODY_PRESETS.get(preset_name, BODY_PRESETS["Full Body"])
    return gr.update(visible=False), preset_data["threshold"]
...
""")
                    ...
                    with gr.Row():
                        body_preset = gr.Dropdown(
                            label="Body Part Preset",
                            choices=list(BODY_PRESETS.keys()),
                            value="Full Body",
                            info="Select what you want to mask"
                        )
                    ...
                    body_custom_prompt = gr.Textbox(
                        label="Custom Detection Prompt",
                        placeholder="Enter custom prompt like: shirt . jacket . coat",
                        visible=False,
                        info="Use ' . ' to separate multiple objects"
                    )
                    
                    with gr.Row():
                        body_dilation = gr.Slider(
                            label="Mask Expansion",
                            minimum=0,
                            maximum=50,
                            value=10,
                            step=1,
                            info="Expand mask edges (recommended: 5-15)"
                        )
                        body_combine_masks = gr.Checkbox(
                            label="Also Show Combined Mask",
                            value=False,
                            info="Add option to select all regions combined",
                    ...
                    # Event handlers
                    body_preset.change(
                        fn=apply_body_preset,
                        inputs=[body_preset, body_threshold],
                        outputs=[body_custom_prompt, body_threshold],
                        show_progress=False
                    )
            ...
            # Create stub variables for disabled tabs to satisfy UI process requirements
            # Single Image tab stubs (8 values)
            sam_inpaint_upload_enable = gr.Checkbox(value=False, visible=False)
            sam_single_image_process = (
                sam_inpaint_upload_enable, gr.Checkbox(value=False, visible=False), gr.Radio(value="0", choices=["0"], visible=False),
                gr.State(value=None), gr.State(value=None), gr.State(value=None),
                gr.Checkbox(value=False, visible=False), gr.State(value=None))
            ui_process += sam_single_image_process
            ...
            # Stubs for other tab components referenced by buttons
            cnet_seg_enable_copy = gr.Checkbox(value=False, visible=False)
            crop_inpaint_enable = gr.Checkbox(value=False, visible=False)
            sam_dummy_component = gr.Label(visible=False)
    ...
    def process(self, p: StableDiffusionProcessing, *args):
        # Quick Body Mask is a standalone feature and doesn't integrate with generation
        # All the original integration features are disabled, so process() is a no-op
        return
