    ...
    def get_input_and_mask(self, mask_blur):
        ...
        if self.inpaint_upload_enable and self.input_image is not None and self.output_mask_gallery is not None:
            if self.dilation_checkbox and self.dilation_output_gallery is not None:
                ...
                # Handle both standard gallery (with index+3) and Quick Body Mask gallery (index 1 is mask)
                gallery = self.output_mask_gallery
                if isinstance(gallery, list) and len(gallery) > 0:
                    # Quick Body Mask outputs: [preview, mask, cutout] - mask is at index 1
                    # Standard SAM outputs: [img0, img1, img2, mask0, mask1, mask2, ...] - masks start at index 3
                    mask_idx = 1 if len(gallery) == 3 else self.output_chosen_mask + 3
...
class BodyMaskInpaintUnit:
    """Special inpaint unit for Quick Body Mask tab that supports selecting individual bodies."""
    def __init__(self, args: Tuple, is_img2img=False):
        self.is_img2img = is_img2img
        self.inpaint_upload_enable: bool = False
        self.cnet_inpaint_invert: bool = False
        ...
        self.input_image = None
        self.output_mask_gallery: List[Dict] = None
        self.mask_selector: str = "Body 1"  # String like "Body 1", "Body 2", "All Combined"
        self.mask_choices: List[str] = ["Body 1"]
        self.init_body_mask_process(args)
    
    def init_body_mask_process(self, args):
        self.inpaint_upload_enable = args[0]
        self.cnet_inpaint_invert = args[1]
        self.cnet_inpaint_idx = args[2]
        self.input_image = args[3]
        self.output_mask_gallery = args[4]
        self.mask_selector = args[5]  # String selector value
        self.mask_choices = args[6] if args[6] else ["Body 1"]  # List of choices
    
    def get_input_and_mask(self, mask_blur):
        image, mask = None, None
        print(f"[BodyMaskInpaintUnit] get_input_and_mask called: enable={self.inpaint_upload_enable}, input_image={type(self.input_image)}, gallery={len(self.output_mask_gallery) if self.output_mask_gallery else None}")
        
        if not self.inpaint_upload_enable:
            return None, None
            
        if self.input_image is None:
            print("[BodyMaskInpaintUnit] ERROR: input_image is None! Make sure you uploaded an image in the Quick Body Mask tab.")
            return None, None
            
        if self.output_mask_gallery is None or len(self.output_mask_gallery) == 0:
            print("[BodyMaskInpaintUnit] ERROR: No masks in gallery! Generate masks first.")
            return None, None
        
        if self.inpaint_upload_enable and self.input_image is not None and self.output_mask_gallery is not None:
            gallery = self.output_mask_gallery
            if isinstance(gallery, list) and len(gallery) > 0:
                # Gallery format: [preview1, mask1, cutout1, preview2, mask2, cutout2, ...]
                # Each body has 3 images, mask is at index 1, 4, 7, etc. (i*3 + 1)
                try:
                    # Find the index based on the selector string
                    choices = self.mask_choices if isinstance(self.mask_choices, list) else ["Body 1"]
                    selector = self.mask_selector if self.mask_selector else choices[0]
                    ...
                    # Calculate mask position in gallery
                    mask_idx = body_idx * 3 + 1
                    
                    if mask_idx < len(gallery):
                        mask = Image.open(gallery[mask_idx]['name']).convert('L')
                        print(f"Body Mask: Selected '{selector}' (index {body_idx}), mask at gallery position {mask_idx}")
                except (ValueError, IndexError, KeyError, TypeError) as e:
                    print(f"Error getting body mask: {e}")
                    # Fallback to first mask
                    if len(gallery) > 1:
                        mask = Image.open(gallery[1]['name']).convert('L')
...
class SAMProcessUnit:
    def __init__(self, args: Tuple, is_img2img=False):
        ...
        args = args[8:]
        # Quick Body Mask tab (uses special unit for body selection)
        self.body_inpaint_unit = BodyMaskInpaintUnit(args, is_img2img)
    ...
    def set_process_attributes(self, p):
        ...
        # Check Quick Body Mask tab
        if inpaint_image is None:
            inpaint_image, inpaint_mask = self.body_inpaint_unit.get_input_and_mask(inpaint_mask_blur)
            inpaint_cn_num = self.body_inpaint_unit.cnet_inpaint_idx
        
        # Check Crop/Category mask tab
        if inpaint_image is None:
        ...
        if inpaint_image is not None and inpaint_mask is not None:
            print(f"[SAMProcessUnit] Setting inpaint: is_img2img={self.is_img2img}, image_type={type(inpaint_image)}, mask_type={type(inpaint_mask)}")
            if self.is_img2img:
                # Validate image is actually a PIL Image before setting
                if not hasattr(inpaint_image, 'copy'):
                    print(f"[SAMProcessUnit] ERROR: inpaint_image is not a valid PIL Image! Type: {type(inpaint_image)}")
                    return
